d3.csv("content/GFKB_Original_RelativeAbundances.csv", d3.autoType).then(function(data, rows) {
  ...
  var genusData = {};
  for (var i = 0; i < graphData.length; i++) {
    var genusName = (graphData[i].genusName);
    var relativeAbundance = graphData[i].relativeAbundance;
  ...
  function init() {
    pieChart(speciesData) //initializes both pie charts here
    pieChart(genusData)
  }
  ...
  function pieChart(dataSet, graphLevel) {
    ...
    d3.select('#doughnut_chart2').html(''); //adds second div for doughnut_chart2
    ...
    if (graphLevel == "genus") {
     ...
     if(graphlevel == "Species Level") { //changed this to if to see if this changes the way this changes
      var graphSubTitle = document.getElementById("graph_level_title");
    ...
    var radius_doughnut2 = Math.min(width_doughnut,height_doughnut)/4-margin_doughnut;
